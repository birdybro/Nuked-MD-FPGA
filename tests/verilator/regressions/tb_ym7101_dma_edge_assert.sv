...
module tb_ym7101_dma_edge_assert;
  logic MCLK;
  logic [7:0] SD;
  logic [7:0] RD_i;
  logic [7:0] AD_i;
  ...
  integer dma_signal_toggle_count;
  integer ext_timing_toggle_count;
  reg prev_dma;
  reg prev_dma_early;
  reg [3:0] early_hist;
  reg [4:0] prev_ext_obs;
  reg [4:0] ext_obs;
  ...
  initial begin
    ...
    dma_signal_toggle_count = 0;
    ext_timing_toggle_count = 0;
    prev_dma = 1'b0;
    prev_dma_early = 1'b0;
    early_hist = 4'h0;
    prev_ext_obs = 5'h0;
    ...
    for (i = 0; i < 1536; i = i + 1) begin
      ...
      // Repeating burst windows to stress DMA edge sequencing.
      if ((i % 128) < 48) begin
        BG = 1'b0;
        BGACK_i = ((i % 16) < 6) ? 1'b0 : 1'b1;
        MREQ = ((i % 6) < 3) ? 1'b0 : 1'b1;
        IORQ = ((i % 10) < 2) ? 1'b0 : 1'b1;
        AS = ((i % 8) < 4) ? 1'b0 : 1'b1;
        UDS = ((i % 12) < 5) ? 1'b0 : 1'b1;
        LDS = ((i % 14) < 5) ? 1'b0 : 1'b1;
        RW = ((i % 5) < 3) ? 1'b0 : 1'b1;
      end else begin
        BG = 1'b1;
        BGACK_i = 1'b1;
      ...
      M1 = ((i % 29) == 4) ? 1'b0 : 1'b1;
      DTACK_i = ((i % 9) == 0) ? 1'b0 : 1'b1;
      INTAK = ((i % 53) == 11) ? 1'b0 : 1'b1;
      ...
      early_hist = {early_hist[2:0], vdp_dma_oe_early};
      if (vdp_dma)
        dma_seen_count = dma_seen_count + 1;
      if (vdp_dma_oe_early)
        dma_early_seen_count = dma_early_seen_count + 1;
      if (i > 8 && ((vdp_dma != prev_dma) || (vdp_dma_oe_early != prev_dma_early)))
        dma_signal_toggle_count = dma_signal_toggle_count + 1;
      prev_dma = vdp_dma;
      prev_dma_early = vdp_dma_oe_early;

      ext_obs = {UWR, LWR, OE0, CAS0, RAS0};
      if (i > 8 && ext_obs != prev_ext_obs)
        ext_timing_toggle_count = ext_timing_toggle_count + 1;
      prev_ext_obs = ext_obs;
    end

    if ((dma_seen_count + dma_early_seen_count) < 8)
      $fatal(1, "Insufficient DMA visibility (%0d)", dma_seen_count + dma_early_seen_count);
    if (dma_signal_toggle_count < 2)
      $fatal(1, "Insufficient DMA/early toggle activity (%0d)", dma_signal_toggle_count);
    if (ext_timing_toggle_count < 16)
      $fatal(1, "Insufficient external timing transitions (%0d)", ext_timing_toggle_count);

    $display("tb_ym7101_dma_edge_assert: PASS");
    $finish;
  end
endmodule
